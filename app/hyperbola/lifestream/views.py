...
def index(request, page=1):
    ...
    if pager.has_previous():
        if pager.previous_page_number() == 1:
            ...
            newer = reverse("lifestream-index-paged",
                            args=[pager.previous_page_number()])
    if pager.has_next():
        older = reverse("lifestream-index-paged",
                        args=[pager.next_page_number()])
...
def archive(request, year, month, page=1):
    ...
    if pager.has_previous():
        if pager.previous_page_number() == 1:
            ...
            newer = reverse("lifestream-archvive-paged",
                            args=[year, month, pager.previous_page_number()])
    if pager.has_next():
        older = reverse("lifestream-archive-paged",
                        args=[year, month, pager.next_page_number()])
...
def hashtag(request, tag, page=1):
    ...
    if pager.has_previous():
        if pager.previous_page_number() == 1:
            ...
            newer = reverse("lifestream-hashtag-paged",
                            args=[tag, pager.previous_page_number()])
    if pager.has_next():
        older = reverse("lifestream-hashtag-paged",
                        args=[tag, pager.next_page_number()])
...
def permalink(request, entry_id):
    try:
        post = LifeStreamItem.objects.select_related('lifestreampicture').get(
            pk=entry_id)
