module "hyperbola_email_dns" {
  source = "../modules/hyperbola/gsuite/email-dns"

  zone_id                      = aws_route53_zone.hyperbola.zone_id
  zone_name                    = aws_route53_zone.hyperbola.name
  google_site_verifcation_keys = "Kt2HDssbfMv5OIL422wGGexn00n1W4nTAZZTfUkyig8,vBbksaHJiPR9xr5eyVdQdvvIixg9di8BLwku3Sr1KCU"
  dkim                         = "p="
}

module "hyperbolausercontent_email_dns" {
  source = "../modules/hyperbola/gsuite/email-dns"

  zone_id                      = aws_route53_zone.hyperbolausercontent.zone_id
  zone_name                    = aws_route53_zone.hyperbolausercontent.name
  google_site_verifcation_keys = "BAsDzAuut0wmAAA4v5iVIiTCWFYu0gLTvkK_sRk5O8Y"
  dkim                         = "p="
}

module "hyperboladc_email_dns" {
  source = "../modules/hyperbola/gsuite/email-dns"

  zone_id                      = aws_route53_zone.hyperboladc.zone_id
  zone_name                    = aws_route53_zone.hyperboladc.name
  google_site_verifcation_keys = "MBJltQtisR_DfhKzGfjs4mjMRpPK-1nRugHrza1elAA"
  dkim                         = "p="
}

